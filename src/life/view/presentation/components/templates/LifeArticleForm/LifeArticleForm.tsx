...
import {LifeArticleDetailResponse} from "src/life/domain/LifeArticleDetailResponse";
...
interface LifeArticleRequestDto {
  seq: number;
  title: string;
  slug: string;
  content: string;
...
interface Props {
  isUpdating?: boolean;
  initialValues?: LifeArticleDetailResponse;
  onSubmit(request: LifeArticleRequestDto): Promise<void>;
}

const LifeArticleForm = ({ isUpdating, initialValues, onSubmit }: Props) => {
  const theme = useTheme();
  const styles = useMemo(() => styleObjects(theme), [theme]);

  return <Formik<LifeArticleRequestDto>
    enableReinitialize
    initialValues={{
...
export default LifeArticleForm;
