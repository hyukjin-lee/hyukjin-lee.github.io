...
import {WorkArticleDetailResponse} from "src/work/domain/WorkArticleDetailResponse";
...
interface WorkArticleRequestDto {
  seq: number;
  title: string;
  slug: string;
  content: string;
...
interface Props {
  isUpdating?: boolean;
  initialValues?: WorkArticleDetailResponse;
  onSubmit(request: WorkArticleRequestDto): Promise<void>;
}

const WorkArticleForm = ({ isUpdating, initialValues, onSubmit }: Props) => {
  const theme = useTheme();
  const styles = useMemo(() => styleObjects(theme), [theme]);

  return <Formik<WorkArticleRequestDto>
    enableReinitialize
    initialValues={{
...
export default WorkArticleForm;
